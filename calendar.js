...
class CalendarScheduler {
        ...
        schedulingSection.innerHTML = `
            <div class="container">
                <div class="section-header">
                    <h2 class="section-title">Agendar Manutenção</h2>
                    <p class="section-subtitle">Escolha o melhor dia e horário para você</p>
                </div>
                ...
                <div class="appointment-form" id="bookingForm" style="display: none;">
                    ...
                    <div class="selected-info">
                        ...
                        <form id="scheduleForm">
                            ...
                            <div class="form-group">
                                <input type="text" id="petName" placeholder="Veículo" required>
                            </div>
                            <div class="form-group">
                                <select id="serviceType" required>
                                    <option value="">Selecione o serviço</option>
                                    <option value="opc-1">Instalação Completa</option>
                                    <option value="opc-2">Manutenção Preventiva</option>
                                    <option value="opc-3">Diagnóstico Preciso</option>
                                    <option value="opc-4">Manutenção Corretiva</option>
                                    <option value="opc-5">Serviços Especializados</option>
                                </select>
                            </div>
                            <div class="form-group">
    ...
    selectTime(time, buttonElement) {
        ...
        if (time === 'DIA_FREELANCER') {
            ...
            document.getElementById('petName').placeholder = "Nome (opcional)";
            document.getElementById('petName').required = false;
        } else {
            // Resetar para serviços por hora
            document.getElementById('serviceType').value = '';
            document.getElementById('petName').placeholder = "Veículo";
    ...
    updateFormForServiceType() {
        ...
        serviceSelect.addEventListener('change', () => {
            if (serviceSelect.value === 'dia-freelancer') {
                ...
                petNameField.placeholder = "Veículo";
    ...
    submitBooking() {
        ...
        if (formData.service === 'dia-freelancer') {
            message = `🐾 *AGENDAMENTO* 🐾\n\n`;
            message += `Olá Jair! Gostaria de contratar seus serviços.\n\n`;
            ...
            message = `🐾 *AGENDAMENTO DE MANTENÇÃO* 🐾\n\n`;
            message += `Olá Jair! Gostaria de agendar um horário.\n\n`;
            message += `*Data:* ${formattedDate}\n`;
            message += `*Horário:* ${this.selectedTime}\n`;
            message += `*Nome:* ${formData.name}\n`;
            message += `*Telefone:* ${formData.phone}\n`;
            message += `*Veiculo:* ${formData.petName}\n`;
            message += `*Serviço:* ${this.getServiceName(formData.service)}\n`;
            if (formData.observations) {
                message += `*Observações:* ${formData.observations}\n`;
            }
    ...
    getServiceName(serviceValue) {
        const services = {
            "opc-1": "Instalação Completa",
            "opc-2": "Manutenção Preventiva",
            "opc-3": "Diagnóstico Preciso",
            "opc-4": "Manutenção Corretiva",
            "opc-5": "Serviços Especializados"			 
        };
